...
// Ensure char and uint8_t are the same size
_Static_assert(sizeof(char) == sizeof(uint8_t), "char and uint8_t must be the same size!");

// Terminal driver management
static const display_driver_t** drivers = NULL;
static bool* active_drivers = NULL;
static size_t driver_count = 0;

// Initialize terminal with available display drivers
void terminal_init(const display_driver_t** display_drivers, size_t arraySize) {
    ...
    for(size_t i = 0; i < arraySize; i++) {
        active_drivers[i] = drivers[i]->check();
        if(active_drivers[i]) {
            drivers[i]->activate();
            drivers[i]->clear();
        }
...
#define TS(x) #x
// Macro to execute a function for all active drivers
...
// Output a character to all active display drivers
void terminal_putc(char c) {
    FOR_ALL_ACTIVE_DRIVERS(driver, driver->putc(c));
}

// Output a string to all active display drivers
...
// Clear all active display drivers
void terminal_clear() {
    FOR_ALL_ACTIVE_DRIVERS(driver, driver->clear());
}

// Test color output for all supported colors
